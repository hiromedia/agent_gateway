...
def get_agents_console():
    ...
    js_code = 'var agents = {},agentType;$(document).ready(function(){$("#connectorslist :checkbox").change(function() {if($(this).is(":checked")) { var selectedAgent = this; var agentName = $(this).val(); $.ajax({type: "post",url: "/v1/agents/" + $(this).val() + "/config",contentType: "application/json;charset=utf-8",dataType: "json",success: function(data) {if (data) { agents[agentName] = data; var domains,bid; bid = Number(data.ext.price.replace("USD/1M",""))/1000; $("#bid").val(bid); if (data.hostFilter && data.hostFilter.exclude) {agentType = 0;domains = data.hostFilter.exclude;} else if (data.hostFilter && data.hostFilter.include) {agentType = 1;domains = data.hostFilter.include;}if (domains){$("input[name=domainstype]")[agentType].checked = true;$("#domainsList").val(JSON.stringify(domains, null,2));}} else {alert("ERROR! cannot load this agent config!"); $(selectedAgent).prop("checked",false);}}});}}); $("#updateBtn").click(function() {if ($("#domainsList").val() || $("#bid").val()) {$.each($("#connectorslist :checkbox:checked"), function() {var agentName = $(this).val();var agentConfig = agents[agentName];if ($("#bid").val())agentConfig.ext.price = ($("#bid").val() * 1000) + "USD/1M";if ($("#domainsList").val()) {var filterType = "include";if ($("input[name=domainstype]:checked").val() == "Ron"){filterType = "exclude";} agentConfig["hostFilter"] = {}; agentConfig["urlFilter"] = {}; agentConfig.hostFilter[filterType] = JSON.parse($("#domainsList").val());agentConfig.urlFilter[filterType] = JSON.parse($("#domainsList").val());}$.ajax({type: "post",url: "/v1/agents/" + agentName + "/restart?executable=basic_bidding_agent&N=' + account_title + '.rtb1." + agentName + "&B=/home/rtbkit/prod/rtb/configs/bootstrap.' + account_title + '.json",contentType: "application/json;charset=utf-8",dataType: "json",data: JSON.stringify(agentConfig, null, 2),success: function(data) {console.log("Update has been succeeded!");},error: function(jqXHR) {try {alert($.parseJSON(jqXHR.responseText).resultDescription);} catch (e) {}}});});}});});'

    return '<html><head><meta charset="UTF-8"><title>' + bidder_title + ' RTBKit</title><style>' + style_classes + '</style><script src="http://code.jquery.com/jquery-1.9.0.min.js" type="text/javascript"></script><script type="text/javascript">' + js_code + '</script></head><body><center><h1>' + bidder_title + '</h1></center><div class="main-container"><div class="left-side"><div id="connectorslist">' + bidders_checklist + '</div></div><div class="right-side"><label for="bid">Bid (1/1000):</label><input type="text" size="10" id="bid" style="margin-left: 10px;"><br/><br/><label for="domainsList">Domains List:</label><input type="radio" name="domainstype" value="Ron" style="margin-left: 15px;">Ron &nbsp;<input type="radio" name="domainstype" value="White"> White<br><textarea id="domainsList" rows="40" cols="60"></textarea></div><div class="footer"><button id="updateBtn" class="action-button animate green">Update</button></div></div></body></html>'
