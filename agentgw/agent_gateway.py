...
def get_agents_config():
    ...
    style_classes = '<style>.animate{transition:all 0.1s;-webkit-transition:all 0.1s;}.action-button{position:relative;margin:0px 10px 10px 0px;border-radius:5px;font-size:15px;color:#FFF;text-decoration:none;}.blue{background-color:#3498DB;border-bottom:5px solid #2980B9;text-shadow:0px -2px #2980B9;}.red{background-color:#E74C3C;border-bottom:5px solid #BD3E31;text-shadow:0px -2px #BD3E31;}.green{background-color:#82BF56;border-bottom:5px solid #669644;text-shadow:0px -2px #669644;}.yellow{background-color:#F2CF66;border-bottom:5px solid #D1B358;text-shadow:0px -2px #D1B358;}.action-button:active{transform:translate(0px,5px);-webkit-transform:translate(0px,5px);border-bottom:1px solid;}div#agents-container { width: 1000px; margin-left: auto; margin-right:auto; }.ui-dialog .ui-state-error { padding: .3em; }.validateTips { border: 1px solid transparent; padding: 0.3em; }</style><link rel="stylesheet" href="http://code.jquery.com/ui/1.12.0/themes/base/jquery-ui.css">'
    js_scripts = '<script src="http://code.jquery.com/jquery-1.9.0.min.js" type="text/javascript"></script><script src="http://code.jquery.com/ui/1.12.0/jquery-ui.js"></script>'

    js_code = '<script type="text/javascript">var agentConfig="";$(document).ready(function(){ var dialog, form, agentName = $( "#agentName" ), tips = $( ".validateTips" ); $("#getConfigBtn").click(function(){if($("#agentsList").val()){$.ajax({type:"post",url:"/v1/agents/" + $("#agentsList").val() + "/config",contentType:"application/json;charset=utf-8",dataType:"json",success: function(data){agentConfig=data; agentName.val($("#agentsList").val()); $("#requestJson").val(JSON.stringify(data,null, 2));}});}});$("#restartBtn").click(function(){if($("#agentsList").val()){$.ajax({type: "post",url: "/v1/agents/" + $("#agentsList").val() + "/config",contentType:"application/json; charset=utf-8",dataType: "json",success: function(data){agentConfig=data;$.ajax({type: "post",url: "/v1/agents/" + $("#agentsList").val() + "/restart?executable=basic_bidding_agent&N=' + account_title + '.rtb1." + $("#agentsList").val() + "&B=/home/rtbkit/prod/rtb/configs/bootstrap.' + account_title + '.json",contentType: "application/json;charset=utf-8",dataType: "json",data: JSON.stringify(data, null, 2),success: function () {alert("Restart has been succeeded!");}});}});}});$("#updateBtn").click(function() {if($("#agentsList").val() && $("#requestJson").val() && agentConfig != "") {$.ajax({type: "post",url: "/v1/agents/" + $("#agentsList").val() + "/restart?executable=basic_bidding_agent&N=' + account_title + '.rtb1." + $("#agentsList").val() + "&B=/home/rtbkit/prod/rtb/configs/bootstrap.' + account_title + '.json",contentType: "application/json;charset=utf-8",dataType: "json",data: $("#requestJson").val(),success: function (data) {alert("Update has been succeeded!");},error: function(jqXHR){try{alert($.parseJSON(jqXHR.responseText).resultDescription);}catch(e){}}});}});$("#stopBtn").click(function() {if ($("#agentsList").val()) {$.ajax({type: "post",url: "/v1/agents/" + $("#agentsList").val() + "/stop?signal=15",contentType: "application/json; charset=utf-8",dataType: "json",data: {},success: function(data) {agentConfig = data;$("#requestJson").val(JSON.stringify(data, null, 2));}});}}); function updateTips( t ) {tips.text( t ).addClass( "ui-state-highlight" );setTimeout(function() { tips.removeClass( "ui-state-highlight", 1500 );}, 500 );}function checkLength( o, n, min, max ) {if ( o.val().length > max || o.val().length < min ) {o.addClass( "ui-state-error" );updateTips( "Length of " + n + " must be between " + min + " and " + max + "." );return false;} else {return true;}}function createAgent() {var valid = true;agentName.removeClass( "ui-state-error" );valid = valid && checkLength( agentName, "agent name", 1, 50 );valid = valid && $("#requestJson").val(); $("#agentsList > option").each(function() {var name = this.value;if (name == agentName.val()) {updateTips("Agent name: " + name + " already exists!");valid = false;}});if ( valid ) {$.ajax({type: "post",url: "/v1/agents/" + agentName.val() + "/start?executable=basic_bidding_agent&N=' + account_title + '.rtb1." + agentName.val() + "&B=/home/rtbkit/prod/rtb/configs/bootstrap.' + account_title + '.json",contentType: "application/json;charset=utf-8",dataType: "json",data: $("#requestJson").val(),success: function(data){dialog.dialog( "close" );location.reload(true);},error: function(jqXHR) {try {dialog.dialog( "close" );alert($.parseJSON(jqXHR.responseText).resultDescription);} catch (e) {}}});   }}dialog = $( "#dialog-form" ).dialog({autoOpen: false,height: 330,width: 350,modal: true,buttons: {"Create an agent": createAgent,Cancel: function() { dialog.dialog( "close" );}},close: function() { form[ 0 ].reset(); agentName.removeClass( "ui-state-error" );}});form = dialog.find( "form" ).on( "submit", function( event ) { event.preventDefault(); createAgent();});$( "#createAgent" ).button().on( "click", function() {  dialog.dialog( "open" );});});</script>'

    html_body = '</head><body><div id="dialog-form" title="Create new agent"><p class="validateTips">All form fields are required.</p><form><fieldset><label for="agentName">Agent name:</label><input type="text" name="agentName" id="agentName" value="New-Agent" class="text ui-widget-content ui-corner-all" style="width: 280;"><input type="submit" tabindex="-1" style="position:absolute; top:-500px"></fieldset></form></div><div id="agents-container" class="ui-widget" ><center><h1>' + bidder_title + '</h1></center><select id="agentsList">' + bidders_combo + '</select>&nbsp;&nbsp;<button id="getConfigBtn" class="action-button animate blue">Get Config</button>&nbsp;&nbsp;<button id="stopBtn" class="action-button animate red">Stop</button>&nbsp;&nbsp;<button id="restartBtn" class="action-button animate yellow">Restart</button>&nbsp;&nbsp;<button id="createAgent">Duplicate/New</button><br/><br/><label for="requestJson">Request JSON</label> <br/><textarea id="requestJson" rows="40" cols="120" style="border-top-width:2px;"></textarea><br/><br/><button id="updateBtn" class="action-button animate green">Update</button></div></body></html>'

    return '<html><head><meta charset="UTF-8"><title>' + bidder_title + ' RTBKit</title>' + style_classes + js_scripts + js_code + html_body
